...
KM.weight <- ggplot(KM_weight, aes(y = weight, fill = artefact.state)) + 
             ...
             geom_text(aes(y = mean_weight, x = 12.5, label = round(mean_weight, 3)), nudge_y = 0.004) +
             scale_fill_manual(values = wes_palette(n = 5, name = "Zissou1"), labels = tag) 
...
S.length <- ggplot(S_dim, aes(x = length, fill = artefact.state)) + 
            ...
            geom_text(aes(y = mean_S_length, x = 53, label = round(mean_S_length, 2)), nudge_y = -49.7) +
...
S.width <- ggplot(S_dim, aes(x = width, fill = artefact.state)) + 
           ...
           geom_text(aes(y = mean_S_width, x = 29, label = round(mean_S_width, 1)), nudge_y = -27) +
           scale_fill_manual(values = wes_palette(n = 1, name = "Zissou1"), labels = tag)
...
S.thickness <- ggplot(S_dim, aes(y = thickness, fill = artefact.state)) + 
               ...
               geom_text(aes(y = mean_S_thickness, x = 1.2, label = round(mean_S_thickness, 1)), nudge_y = -0.2) +
               scale_fill_manual(values = wes_palette(n = 1, name = "Zissou1"), labels = tag)
...
S.weight <- ggplot(S_weight, aes(y = weight, fill = artefact.state)) + 
             ...
             geom_text(aes(y = mean_S_weight, x= 0.85, label = round(mean_S_weight, 3)), nudge_y = -0.001) +
             scale_fill_manual(values = wes_palette(n =3, name = "Zissou1"), labels = tag) 
...
LSS.weight <- ggplot(LSS_weight, aes(y = weight, fill = artefact.state)) + 
             ...
             geom_text(aes(y = mean_LSS_weight, x= 11.5, label = round(mean_LSS_weight, 3)), nudge_y = 0.0007) +
             scale_fill_manual(values = wes_palette(n =3, name = "Zissou1"), labels = tag) 
...
KM.raw_material <- ggplot(data = KM_raw_material) + aes(x = raw.material, fill = raw.material) + 
     ...
     theme(legend.position = "none") + 
     labs(x = " ", y = "N") + 
     scale_x_discrete(labels=c("Baltic flint", "other", "Lydite")) +
...
PS.raw_material <- ggplot(data = PS_raw_material) + aes(x = raw.material, fill = raw.material) + 
     ...
     theme(legend.position = "none") + 
     labs(x = " ", y = "N") + 
     scale_x_discrete(labels= "Lydite") +
...
PS.back_morpho <- ggplot(data = PS_back) + aes(x = morphology.back, fill = morphology.back) + 
     ...
     theme(legend.position = "none") + 
     labs(x = " ", y = "N") + 
     scale_x_discrete(labels=c("clear blunting", "cortex + partly retouched", "cortex/unworked", "partly retouched", "retouched")) +
...
# Defines only the rows with complete Keilmesser  
KM_lateralisation <- KM_lateralisation[1:111, ]
...
PS_lateralisation <- PS_lateralisation[c(1:9, 11:15, 17:24), ]
