---
title: 'Summary stats - Lithic analysis from three sites: Balver Höhle, Buhlen & Ramioul'
author: "Lisa Schunk"
date: "`r Sys.Date()`"
output:
  html_document:
...
dir_out <- "analysis/all_sites/summary_stats/"
...
# Load packages
```{r Libraries}
pack_to_load <- c("tools", "openxlsx", "doBy", "R.utils")
...
```{r}
imp_data <- loadObject(paste0(dir_in, "all_sites_analysis.Rbin"))
...
### Dimensions
```{r}
# Dimensions Keilmesser, Keilmesser-points, Pradnik scraper & 
# Lateral sharpening spall 
dimensions <- summaryBy(length + width + thickness ~ technological.class +
              artefact.state, data = imp_data, FUN = nminmaxmeanmedsd)
str(dimensions)
...
```{r}
file_out <- "all_sites_analysis_stats"
...
```{r}
file_out <- c(paste0(dir_out, file_out, ".xlsx"), paste0(dir_out, 
            file_out, ".Rbin"))
