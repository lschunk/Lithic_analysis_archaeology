...
KM.length <- ggplot(KM_dim, aes(x = length, fill = artefact.state)) + 
             ...
             geom_text(aes(y = mean_length, x = 67, label = round(mean_length, 1)), nudge_y = -38) +
...
KM.width <- ggplot(KM_dim, aes(x = width, fill = artefact.state)) + 
            ...
            geom_text(aes(y = mean_length, x = 42, label = round(mean_width, 1)), nudge_y = -34) +
            scale_fill_manual(values = wes_palette(n = 5, name = "Zissou1"), labels = tag) 
...
KM.thickness  <- ggplot(KM_dim, aes(y = thickness, fill = artefact.state)) + 
                 ...
                 geom_text(aes(y = mean_thickness, x = 37.5, label = round(mean_thickness, 1)), nudge_y = 1) +
                 scale_fill_manual(values = wes_palette(n = 5, name = "Zissou1"), labels = tag) 
...
KM_back <- KM_back[-c(279, 301:317, 326, 328), ]
...
KM.back  <- ggplot(KM_back, aes(y = thickness.back, fill = artefact.state)) + 
                 ...
                 geom_text(aes(y = mean_KM_back, x = 28.5, label = round(mean_KM_back, 1)), nudge_y = -0.7) +
...
PS.length <- ggplot(PS_dim, aes(x = length, fill = artefact.state)) + 
             ...
             geom_text(aes(y = mean_PS_length, x = 52, label = round(mean_PS_length, 1)), nudge_y = -40.4) +
...
PS.width <- ggplot(PS_dim, aes(x = width, fill = artefact.state)) + 
            ...
            geom_text(aes(y = mean_PS_width, x = 35, label = round(mean_PS_width, 2)), nudge_y = -24.1) +
            scale_fill_manual(values = wes_palette(n = 1, name = "FantasticFox1"), labels = tag) 
...
PS.thickness <- ggplot(PS_dim, aes(y = thickness, fill = artefact.state)) + 
                ...
                geom_text(aes(y = mean_PS_thickness, x = 6.8, label = round(mean_PS_thickness, 1)), nudge_y = -0.5) +
                scale_fill_manual(values = wes_palette(n = 1 , name = "FantasticFox1"), labels = tag) 
...
PS.back  <- ggplot(PS_back, aes(y = thickness.back, fill = artefact.state)) + 
                 ...
                 geom_text(aes(y = mean_PS_back, x = 6.8, label = round(mean_PS_back, 1)), nudge_y = -0.5) +
                 scale_fill_manual(values = wes_palette(n = 3, name = "Zissou1"), labels = tag) 
...
LSS.length <- ggplot(LSS_dim, aes(x = length, fill = artefact.state)) + 
              ...
              geom_text(aes(y = mean_LSS_length, x = 35, label = round(mean_LSS_length, 1)), nudge_y = -17.7) +
              scale_fill_manual(values = wes_palette(n = 4, name = "Darjeeling2"), labels = tag) 
...
LSS.width <- ggplot(LSS_dim, aes(x = width, fill = artefact.state)) + 
             ...
             geom_text(aes(y = mean_LSS_width, x = 19.8, label = round(mean_LSS_width, 1)), nudge_y = -1.55) +
             scale_fill_manual(values = wes_palette(n = 4, name = "Darjeeling2"), labels = tag) 
...
LSS.thickness <- ggplot(LSS_dim, aes(y = thickness, fill = artefact.state)) + 
                 ...
                 geom_text(aes(y = mean_LSS_thickness, x = 23.9, label = round(mean_LSS_thickness, 2)), nudge_y = -0.3) +
                 scale_fill_manual(values = wes_palette(n = 4, name = "Darjeeling2"), labels = tag) 
...
n <- doBy::summaryBy(length + width ~ site, data = KM_comp_dim, FUN = length)
tag <- gsub(pattern = "_", replacement = " ", paste0(n[[1]], " (N = ", n[[2]], ")"))
...
KM.length_width <- ggplot(KM_comp_dim, aes(y = length, x = width, fill = site)) +
                   ...
                   labs(x = "length [mm]", y = "width [mm]", title = "", fill = "", size = 12) +
...
# Scatterplot Keilmesser (complete + point) length VS width 
KM.tip.length_width <- ggplot(KM_comp.tip_dim, aes(y = length, x = width, colour = site, shape = artefact.state)) +
                   geom_point(size = 2) +
                   scale_colour_manual(values = wes_palette(n = 3, name = "FantasticFox1", type = "continuous"), labels = tag) +
                   labs(x = "length [mm]", y = "width [mm]", title = "", fill = "", size = 12) +
                   xlim(10, 90) + ylim(0, 160) +
                   theme_classic() 
...
# Pradnick scraper (complete): length-width combined with morpho type
# Load data sheet Pradnick morpho type 
...
PS.width_length_morpho <- ggplot(PS_morpho.type, aes(y = length, x = width, fill = morpho.type)) +
                      ...
                      xlim(0, 65) + ylim(0, 80) +
                      theme_classic() +
                      scale_fill_manual(values = wes_palette(n = 5, name = "FantasticFox1", type = "continuous"), labels = tag) 
                      
print(PS.width_length_morpho)
file_out <- paste0(file_path_sans_ext(info_in[["file"]]), "PS.width_length_morpho", ".pdf")
ggsave(filename = file_out, plot = PS.width_length_morpho, path = dir_out, device = "pdf", width = 170, height = 250, units = "mm")
...
ggsave(filename = file_out, plot = PS.raw_material, path = dir_out, device = "pdf", , width = 210, height = 150, units = "mm")
...
KM_Pradnick.method <- read.xlsx(xlsxFile = data_file, sheet = 17)  
